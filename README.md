# Francisco Marques | Medical Student & Clinical Researcher

🎓 **Medical Student (UEMA)** | 🔬 **Clinical Research & Data Science** | 📊 **Biostatistics**\
📍 Brazil | ✉️ [email](mailto\:francisco@pesquisaemdados.com.br) 

---

## 🛠 Technical Skills

- R (data analysis, statistical modeling)
- Python (data analysis, automation)
- LLMs (prompt design, applied research workflows)
- APIs (integration for research and automation)
- Scientific writing (ABNT, manuscripts)
- Research design (observational studies, clinical research)

---

## 🔬 Research & Professional Experience

### 🏥 Clinical & Epidemiological Research

- Development of clinical and epidemiological studies in Brazil
- Statistical analysis and data visualization
- Collaboration on academic research projects

### 📊 Biostatistics & Data Analysis

- Data cleaning and analysis in R
- Descriptive and inferential statistics
- Preparation of tables, figures, and reports

---

## 📚 Academic Background

- **MD (in progress)** — Universidade Estadual do Maranhão (UEMA)

---

## 📊 Research Interests

- Clinical research
- Internal medicine
- Epidemiology and public health
- LLMs applied to scientific research
- AI-assisted systematic reviews

---

## 📈 Currently Learning

- Advanced statistical modeling
- Scientific communication
- Research methodology
- Artificial intelligence in health
